...
The objective is to outline key enhancements and improvements to Falco to optimize its rule adoption, customization and management capabilities. This proposal aims to address the expectations of adopters, clarify rule contribution processes and criteria, provide guidance on rule customization and tuning, offer references for further learning, and outline use cases for better understanding. Additionally, it introduces a rule maturity framework.
...
- Falco features a CLI tool called [falcoctl](https://github.com/falcosecurity/falcoctl) to manage the lifecycle of rules (installation, updates), see [blogpost](https://falco.org/blog/falcoctl-install-manage-rules-plugins/).
...
- Guidance and templates are provided to assist with the tuning of rules.
...
- Establish specific criteria for each maturity level and define general criteria for creating, contributing, and updating rules based on their respective maturity levels.
- Offer guidance and templates for rules tuning purposes.
- Create a clear PR template for the rules repo that references the criteria for rules.
...
*Example 1: Detect release_agent File Container Escapes*
...
In some scenarios, adopters may benefit from acknowledging that operationalizing detections could require additional data analysis and correlation in data lake systems. This implies that adopting organizations may need to leverage data lake systems for further analysis and contextual correlation of the generated alerts, going beyond rule tuning.
